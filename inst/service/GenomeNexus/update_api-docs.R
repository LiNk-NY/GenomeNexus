# setwd("~/gh/GenomeNexus")
file_loc <- "inst/service/GenomeNexus/api.json"

download.file(
    url = "https://www.genomenexus.org/v2/api-docs",
    destfile = file_loc
)

md5 <- digest::digest(file_loc, file = TRUE)
nexuslines <- readLines("R/GenomeNexus.R")
...
if (!identical(oldmd5, md5)) {
    writeLines(updatedlines, con = file("R/GenomeNexus.R"))
    quit(status = 0)
} else {
